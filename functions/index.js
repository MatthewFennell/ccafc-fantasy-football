...
exports.updateTeam = functions
    ...
    .https.onCall((data, context) => {
        ...
        return activeTeamRef.get()
            .then(activeDocs => activeDocs.docs.map(doc => ({ player_ids: doc.data().player_ids, id: doc.id })))
            .then(currentTeam => {
                ...
                const newPlayers = data.playersToAdd.filter(p => !myActiveTeamIds.includes(p));
                const removedPlayers = data.playersToRemove.filter(p => myActiveTeamIds.includes(p));
                const newTeam = myActiveTeamIds.filter(p => !data.playersToRemove.includes(p)).concat(newPlayers);
                ...
                return Promise.all(promises).then(players => {
                    ...
                    db.collection('active-teams').doc(activeTeamId).update({
                        player_ids: newTeam
                    ...
                    // For each player in the active team, remove if they aren't in the new team
                        ...
                        .then(() => {
                            ...
                            Promise.all(playersToRemovePromises).then(prices => {
                                const priceOfNewPlayers = players.filter(p => newPlayers.includes(p.id))
                                    .reduce((acc, curVal) => acc + curVal.data.price, 0);
                                const priceOfRemovedPlayers = prices.reduce((acc, curVal) => acc + curVal, 0);
                                db.collection('users').doc(context.auth.uid).get().then(
                                    user => {
                                        ...
                                        const pointsPenalty = Math.max((numberOfTransfers - remainingTransfers) * constants.transferPointPenalty, 0);
                                        db.collection('users').doc(context.auth.uid).update({
                                            remaining_budget: admin.firestore.FieldValue.increment(priceOfRemovedPlayers - priceOfNewPlayers),
                                            total_points: admin.firestore.FieldValue.increment(pointsPenalty * -1),
                                            remaining_transfers: Math.max(0, remainingTransfers - numberOfTransfers)
                                        });
                                    }
                                );
                            });
