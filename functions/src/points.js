...
exports.pointsForWeek = functions
    ...
    .https.onCall((data, context) => {
        ...
        return common.getCorrectYear(db).collection('weekly-teams').where('user_id', '==', data.userId)
            ...
            .then(
                playerIds => {
                    ...
                    playerIds.map(playerId => playerPromises.push(common.getCorrectYear(db).collection('weekly-players')
                        .where('player_id', '==', playerId).where('week', '==', data.week)
            ...
            .then(
                players => {
                    const playerPromises = [];
                    players.map(player => playerPromises.push(common.getCorrectYear(db).collection('players').doc(player.player_id).get()
                        .then(result => ({
                            ...player,
    ...
    getDisplayName(uid).then(user => {
        common.getCorrectYear(db).collection('results-history').doc(constants.resultsHistoryId).get()
            .then(doc => {
                ...
                return common.getCorrectYear(db).collection('results-history').doc(constants.resultsHistoryId).update({
...
exports.submitResult = functions
    ...
    .https.onCall((data, context) => common.hasPermission(context.auth.uid,
        ...
        common.getCorrectYear(db).collection('teams').doc(data.team).get()
            .then(team => {
                if (team.exists) {
                    team.ref.update({
        ...
        playerIds.map(playerId => playerPromises.push(common.getCorrectYear(db).collection('players')
            .doc(playerId).get()
            .then(player => {
                if (!player.exists) {
        ...
        return Promise.all(playerPromises).then(playerPositionsArray => {
            ...
            playerIds.forEach(playerId => {
                ...
                common.getCorrectYear(db).collection('player-points').where('player_id', '==', playerId).where('week', '==', data.week)
                    .get()
                    .then(playerDocs => {
                        if (playerDocs.empty) {
                            common.getCorrectYear(db).collection('player-points').add({
                                player_id: playerId,
                                week: data.week,
                                goals,
                                assists,
...
exports.teamStatsByWeek = functions
    ...
    .https.onCall((data, context) => {
        common.isAuthenticated(context);
        return common.getCorrectYear(db).collection('teams').doc(data.teamId).get()
            .then(result => {
                if (result.exists) {
            ...
            .then(team => common.getCorrectYear(db).collection('player-points')
    ...
    getDisplayName(uid).then(user => {
        common.getCorrectYear(db).collection('results-history').doc(constants.resultsHistoryId).get()
            .then(doc => {
                ...
                uniqueKeys.map(key => playerPromises.push(common.getCorrectYear(db).collection('players').doc(data[key]).get()
                    .then(player => {
    ...
    .https.onCall((data, context) => common.hasPermission(context.auth.uid,
        ...
        .then(() => {
            ...
            uniqueKeys.forEach(id => {
                ...
                common.getCorrectYear(db).collection('player-points').where('player_id', '==', id).where('week', '==', data.gameWeek)
                    .get()
                    .then(playerDocs => {
                        if (playerDocs.empty) {
                            common.getCorrectYear(db).collection('players').doc(id).get()
                                .then(player => {
                                    common.getCorrectYear(db).collection('player-points').add({
                                        player_id: id,
                                        week: data.gameWeek,
