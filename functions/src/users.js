...
exports.userInfoForWeek = functions
    ...
    .https.onCall((data, context) => {
        ...
        return common.getCorrectYear(db).collection('weekly-teams').where('week', '==', data.week).where('user_id', '==', data.userId)
            .get()
            .then(
                query => {
                    if (query.size > 1) {
            ...
            .then(result => common.getCorrectYear(db).collection('weekly-teams').where('week', '==', data.week).get()
                .then(weeklyDocs => {
...
exports.userStats = functions
    ...
    .https.onCall((data, context) => {
        common.isAuthenticated(context);
        return common.getCorrectYear(db).collection('users').doc(data.userId).get()
            .then(
                user => {
                    if (!user.exists) {
                        return ({
