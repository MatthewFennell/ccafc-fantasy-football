...
exports.triggerWeeklyTeams = functions
    ...
    .https.onCall((data, context) => common.hasPermission(context.auth.uid,
        constants.PERMISSIONS.TRIGGER_WEEK).then(() => common.getCorrectYear(db).collection('application-info').doc(constants.applicationInfoId).get()
        .then(appInfo => {
        ...
        .then(() => common.getCorrectYear(db).collection('active-teams').get()
            .then(querySnapshot => {
                ...
                querySnapshot.docs.forEach((doc, index) => {
                    ...
                    const weeklyTeamRef = common.getCorrectYear(db).collection('weekly-teams').doc();
